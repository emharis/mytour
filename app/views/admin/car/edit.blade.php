...
<div class="row">
    <div class="col-xs-12">
        ...
        <div>
            ...
            <form class="form-horizontal" method="POST" action="{{URL::to('admin/paket/car/edit')}}" enctype="multipart/form-data" >
                ...
                <table class="table table-bordered">
                    <tbody>
                        ...
                        <tr>
                            <td>Nama</td>
                            <td colspan="2">
                                <input value="{{$car->nama}}" name="nama" type="text" placeholder="Nama" class="col-sm-12"  required />
                            </td>
                        </tr>
                        <tr>
                            <td>Harga</td>
                            <td colspan="2">
                                <input value="{{$car->price}}" name="price"  type="number" placeholder="Price" class="col-sm-2 uang"  />
                            </td>
                        </tr>
                        <tr>
                            <td>Deskripsi</td>
                            <td colspan="2">
                                <textarea name="desc" class="full">{{$car->desc}}</textarea>
                            </td>
                        </tr>
                        <tr>
                            <td>Publish</td>
                            <td colspan="2">
                                <select name="publish">
                                    <option {{($car->publish == 'N' ? 'selected':'')}} value="N">Don't Publish</option>
                                    <option {{($car->publish == 'Y' ? 'selected':'')}} value="Y">Publish it NOW!!!</option>
                                </select>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="col-sm-3 center">
                                    <img src="{{$car->foto_1 != '' ? 'images/car/'.$car->foto_1 : ''}}" class="col-sm-12" id="img-1">
                                    @if($car->foto_1 != '')
                                    <a class="btn btn-danger btn-xs center btn-del-foto" foto-src="1" rel="img-1">Delete</a>
                                    @endif
                                </div>
                                <div class="col-sm-3 center">
                                    <img  src="{{$car->foto_2 != '' ? 'images/car/'.$car->foto_2 : ''}}" class="col-sm-12" id="img-2">
                                    @if($car->foto_2 != '')
                                    <a class="btn btn-danger btn-xs center btn-del-foto" foto-src="2" rel="img-2">Delete</a>
                                    @endif
                                </div>
                                <div class="col-sm-3 center">
                                    <img src="{{$car->foto_3 != '' ? 'images/car/'.$car->foto_3 : ''}}" class="col-sm-12" id="img-3">
                                    @if($car->foto_3 != '')
                                    <a class="btn btn-danger btn-xs center btn-del-foto" foto-src="3" rel="img-3">Delete</a>
                                    @endif
                                </div>
                                <div class="col-sm-3 center">
                                    <img src="{{$car->foto_4 != '' ? 'images/car/'.$car->foto_4 : ''}}" class="col-sm-12" id="img-4">
                                    @if($car->foto_4 != '')
...
//                $('#img_upl_prev').attr('src', ev2.target.result);
        ...
        $('.btn-del-foto').click(function () {
            if (confirm('Anda akan menghapus foto ini?')) {
                ...
                var url = "{{URL::to('admin/paket/car/deletefoto')}}" + "/" + fotonum + "/" + carid;
